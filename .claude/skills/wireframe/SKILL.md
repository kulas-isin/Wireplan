...
"flows": [
  {
    ...
    "edges": [
      ["s","plan"], ["plan","pay","前往付款"],
      ["pay","ok","是"], ["pay","plan","否"],   // [from, to, 標籤?]
      ["ok","e"]
    ]
  }
]
...
- `name`（選）：原名稱口語/冗長時可回傳精煉版（意思不可變）；匯入端只對「未蓋章」的卡改名，並把原名記進備註。
- `talks`（選）：整理 LINE/會議紀錄時，每卡回傳對話陣列 `[{"who":"client|us","text":"精煉一句","date":"YYYY/M/D"}]`；匯入端追加（同文字去重），鎖定卡也可追加（對話是紀錄非規格變更）。
- `elements`（選）：這條需求的畫面需要的元件清單 `["搜尋列","匯出鈕","訂單表格"]`，具體到顆（每個欄位群、每顆按鈕、每個表格）；匯入端追加（去重），鎖定卡也可追加（元件清單是畫面規劃非規格變更）。卡片詳細頁會自動比對已對應 wireframe 上的元件，畫好的打勾、沒畫的標「還沒畫」。
- `unit`（選）：需求所屬的單元/模組名（如「託運單管理」），匯入端只補空的、不覆蓋手動歸檔；單元牆依此分組。
- `pages`（選）：這條需求涵蓋的畫面，含情境分支，每頁附該頁元件磚 `[{"name":"結帳頁","bricks":["訂單明細表","收件人表單","付款鈕"]}]`（純字串頁名也接受）；匯入端依頁名去重追加，磚為文字時自動猜型別。卡片詳細頁的「畫面地圖」會顯示：已建的頁從 wireframe 即時算磚、可跳轉；規劃頁可一鍵建立，磚直接變成頁上元件並自動連結需求。
